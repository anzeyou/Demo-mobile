...
<div class="module_content">
 ...
 $page=@$_GET['page']?$_GET['page']:1;
  $pagesize=2;
  $sql_count="select count(*)  as count from login";
 ...
 for($i=1;$i<=$maxpage;$i++)
 $maxarray[$i]=$i;
$sql="select id,username,password from login limit ".($page-1)*$pagesize.",$pagesize";
$rec=mysql_query($sql);
...
?>
 ...
 <tr>
  <tr>
     <td class="bt">用户名:<input type="text" name="usernm" value="<?php echo $row['username'];?>" /></td>
  </tr>
  <tr>
      <td class="bt">密码:<input type="text" name="passwd"  value="<?php echo $row['password'];?>" /> </td>
   </tr>
   <td  style="padding-left:500px"><a href="#" onclick="javescript:if(confirm('确定删除？')) location='user.php?id=<?php echo $row['ID'];?>'">删除</a></td>
   </tr>
   </form>
   </table>
   <?php
